!function(t, i) {
    "use strict";
    function n(n, o) {
        this.$input = t(n), this.$rating = t("<span></span>").css({cursor: "default"}).insertBefore(this.$input), this.options = function(n) {
    ...
    n.prototype = {_init: function() {
            for (var n = this, s = this.$input, o = this.$rating, a = function(t) {
                ...
                var r = t('<div class="rating-symbol"></div>').css({display: "inline-block", position: "relative"});
                t('<div class="rating-symbol-background ' + this.options.empty + '"></div>').appendTo(r), t('<div class="rating-symbol-foreground"></div>').append("<span></span>").css({display: "inline-block", position: "absolute", overflow: "hidden", left: 0, right: 0, width: 0}).appendTo(r), o.append(r), this.options.extendSymbol.call(r, this._indexToRate(e))
            }
            this._updateRate(s.val()), s.on("change", function() {
                n._updateRate(t(this).val())
            });
            var l, p = function(i) {
                var s = t(i.currentTarget), o = Math.abs((i.pageX || i.originalEvent.touches[0].pageX) - (("rtl" === s.css("direction") && s.width()) + s.offset().left));
            ...
            var i = this.$rating, n = Math.floor(t);
            ...
            var i = Math.ceil(t % 1 * this.options.fractions) / this.options.fractions, n = Math.pow(10, this.options.scale);
            return Math.floor(t) + Math.floor(i * n) / n
        }, _contains: function(t) {
            var i = this.options.step > 0 ? this.options.start : this.options.stop, n = this.options.step > 0 ? this.options.stop : this.options.start;
            return t >= i && n >= t
        ...
        return this.each(function() {
            var a = t(this), e = a.data("rating");
            e || a.data("rating", e = new n(this, i)), "string" == typeof i && "_" !== i[0] && (s = e[i].apply(e, o))
        }), s || this
    }, t.fn.rating.defaults = {filled: "glyphicon glyphicon-star", filledSelected: i, empty: "glyphicon glyphicon-star-empty", start: 0, stop: s, step: 1, fractions: 1, scale: 3, extendSymbol: function(t) {
        }}, t(function() {
